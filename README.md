...
- **波形采样控件**（新增）  
  提供 `SampledWaveformWidget`，可实时接收外部数据（0~1 归一化值）并以滚动波形图形式显示。支持自定义采样间隔和最大振幅比例，适用于实时监控传感器数值或强度变化。

- **样式系统增强**（新增）  
  重构 Qt 样式表，使用 `type` 和 `mode` 属性实现精细的控件分类（导航按钮、操作按钮、标题、输入框等）。支持亮色/暗色主题一键切换，代码中通过 `apply_widget_properties()` 统一设置控件属性，配合 QSS 实现现代化玻璃拟态界面。
...
`SampledWaveformWidget` 是一个实时滚动波形图控件，使用方法如下：

- **添加至 UI**：在 Qt Designer 中提升一个 QWidget 为 `SampledWaveformWidget`，或直接在代码中创建。
- **输入数据**：调用 `input_data(double value)`，其中 `value` 范围为 0.0 ~ 1.0，控件会自动按采样间隔采集并更新波形。
- **调整采样间隔**：通过 `set_sample_interval_ms(int ms)` 设置，默认 20ms。
- **调整振幅比例**：通过 `set_max_amplitude(double ratio)` 限制波形垂直幅度（0.1~1.0），默认 0.8。
...
// 每收到一个传感器数值就调用
wave->input_data(sensorValue); // sensorValue ∈ [0,1]
```
...
<details>
<summary><b>Q14: 端口被占用，无法启动服务</b></summary>
...
<details>
<summary><b>Q15: 手机 App 无法连接到 WebSocket 服务</b></summary>
...
<details>
<summary><b>Q16: 服务启动后控制台没有输出信息</b></summary>
...
<details>
<summary><b>Q17: 配对失败，提示 400 或 401</b></summary>
...
<details>
<summary><b>Q18: 消息发送失败，提示 405</b></summary>
...
<details>
<summary><b>Q19: 日志在 Qt 界面不显示或显示等级不对</b></summary>
...
<details>
<summary><b>Q20: Windows 调试控制台不出现</b></summary>
