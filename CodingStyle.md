...
- [ ] 头文件分区顺序为 `public` → `protected` → `private` → `signals` → `private slots`
- [ ] 各分区内声明按逻辑分组且顺序符合规范（构造/析构、成员函数、成员变量等）
- [ ] 模板函数定义已分离到 `_impl.h`
- [ ] 静态工具函数已移出类声明，放入独立 `_util` 文件
- [ ] 源文件函数定义顺序与头文件完全一致
- [ ] 函数注释符合 Doxygen 风格或简单行内描述
- [ ] 日志输出位置合理、级别恰当，无冗余或高频输出
- [ ] 未改变原有代码的功能、逻辑或命名
- [ ] 所有符号可正常编译，无遗漏分区标签（如缺失的 `private:` 已补全）
