...
class DGLabServer:
    ...
    async def process_command(self, cmd):
        ...
        logger.debug(f"处理命令: {cmd_type}, req_id={req_id}")

        # ---------- 连接相关 ----------
        if cmd_type == "connect":
            logger.info("收到连接请求，开始连接 WebSocket 服务器...")
            ...
            logger.info("收到断开请求，关闭 WebSocket 连接")
            ...
            url = cmd.get("url")
            if url:
                logger.info(f"设置 WebSocket 服务器地址: {url}")
                self.dglab.set_ws_url(url)
                response = {"status": "ok", "message": "设置成功"}
            else:
                logger.warning("set_ws_url 缺少 url 参数")
        ...
        # ---------- 绑定与状态查询 ----------
            ...
            if not target_id:
                logger.warning("bind_target 缺少 target_id 参数")
            ...
            if raw_channel is None or mode is None:
                ...
                if channel is None:
                    ...
                    if mode == 3 or mode == 4:
                        ...
                        if repeat > 100:
                            repeat = 100
                            logger.warning(f"连续次数超过100，已限制为100")
                        base_mode = 0 if mode == 3 else 1  # 0=减少,1=增加
                        logger.info(f"连续{'减少' if base_mode==0 else '增加'} {repeat} 次 (通道{channel})")
                        success = True
                        for i in range(repeat):
                            ...
                            if not ok:
                                ...
                                break
                            if (i + 1) % 10 == 0:
                                logger.debug(f"已执行 {i+1}/{repeat} 次")
                        response = {
                        ...
                        # 单次操作 (0,1,2)
                        logger.info(f"强度操作: 通道={channel}, mode={mode}, value={value}")
        ...
        # ---------- 清空队列 ----------
            ...
            if raw_channel is None:
                logger.warning("clear_queue 缺少 channel 参数")
