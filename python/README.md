...
- **连接管理**：提供同步 `connect()` 方法，自动完成首次连接并等待服务器分配 `client_id`，同时启动后台心跳循环；提供同步 `sync_close()` 方法用于主动断开连接。
...
- 该类基于 `asyncio` 实现，但提供了同步包装方法（如 `connect`、`sync_send_strength_operation`、`sync_close` 等），便于在非异步环境中直接调用。
