...
- **CS2 GSI 模块 (CS2GSIModule)**: 单独封装 CS2 GSI 逻辑——通过 Python 工具 `PathFinder.py` 跨平台查找 CS 游戏目录、随机选取监听端口、按模块最小查询周期计算 buffer/throttle 生成 `gamestate_integration_dglab.cfg`（路径记录到 `user.json` 的 `app.gsi`）；最小周期变化时自动更新配置，若 `cs2.exe` 运行中则提示重启游戏生效，未运行无需提示。
