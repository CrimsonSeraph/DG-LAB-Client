...
- **日志导出**: 新增 `LogExporter` 与导出设置对话框（`LogExportSettingsDialog`）——配置页面“导出日志”按钮将界面日志按设置写入文件：支持导出日志级别过滤（仅指定级别/及以上/以下）、导出位置（目录不存在自动创建、不可用回退默认并警告）、保留日志数量（默认 1，分片视为一份，程序退出时自动清理多余日志）、单个日志大小上限（默认 5MB，超出分片写入多个文件）；设置持久化到 `user.json` 的 `app.log` 下。
